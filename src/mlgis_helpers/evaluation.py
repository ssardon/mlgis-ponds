"""
Evaluation metrics, plotting, and inference-style validation utilities.
...
from mlgis_helpers.data_loading import grid_indices, read_bands_float32


# I. METRIC CALCULATION
# ---------------------

def threshold_at_precision(y_true: np.ndarray,
                           y_score: np.ndarray,
                           target_p: float = 0.90) -> float:
    """Find threshold for target precision (control false positives)."""
...
def calculate_metrics(y_true, y_pred,
                      threshold=0.5, patch_shape=None) -> Dict[str, Any]:
    """
    ...
    if np.isnan(y_pred).any():
        print(f"WARNING: {np.isnan(y_pred).sum()}")
        print("NaN predictions found, replacing with 0.5")
    ...
    if patch_shape is not None:
        try:
            ...
            if len(patch_shape) == 3:  # (B, H, W)
                ...
                print(f"WARNING: Unsupported patch_shape {patch_shape}")
                print("skipping patch-level metrics")
                y_true_patches = None

            if y_true_patches is not None:
                patch_true = np.any(y_true_patches > 0.5,
                                    axis=axes).astype(np.float32)
                patch_scores = np.max(y_pred_patches, axis=axes)

                if len(np.unique(patch_true)) > 1:
                    patch_auc = roc_auc_score(patch_true, patch_scores)

                    patch_bin = (patch_scores > threshold).astype(int)
                    metrics['patch_precision'] = precision_score(patch_true,
                                                                 patch_bin,
                                                                 zero_division=0)
                    metrics['patch_recall'] = recall_score(patch_true,
                                                           patch_bin,
                                                           zero_division=0)
                    metrics['patch_f1'] = f1_score(patch_true,
                                                   patch_bin,
                                                   zero_division=0)
                else:
                    print("WARNING: All patches have same label")
                    print("Cannot compute patch-level AUC")
                    patch_auc = float('nan')
        except Exception as e:
            print(f"Patch metric error: {e}")

    metrics['patch_auc'] = patch_auc

    # Ensure keys exist even if calculation failed (for safe downstream access)
    metrics.setdefault('patch_f1', None)
    metrics.setdefault('patch_precision', None)
    metrics.setdefault('patch_recall', None)
...
def print_summary(results):
    ...
    header = "{:<12} {:<9} {:<10} {:<10} {:<10} {:<10} {:<10}"
    row = "{:<12} {:<9.4f} {:<10.4f} {:<10.4f} {:<10.4f} {:<10.1f} {:<10.1f}"
    print("\n" + "=" * 75)
    print(header.format("Model", "AUC", "Precision", "Recall",
                        "F1", "TrainSec", "PredSec"))
    print("-" * 75)

    for m in results:
        print(row.format(
            m.get("name", "Unknown")[:12],
            m["auc"], m["prec"], m["rec"], m["f1"],
            m.get("train_time", 0.0), m.get("pred_time", 0.0)
        ))

    print("=" * 75)


def write_metrics_txt(out_dir: str,
                      metrics: dict,
                      filename: str = 'test_metrics.txt') -> str:
...
# III. Inference-Consistent Validation
# ------------------------------------
...
def validate_inference_style(model, config, task_config, paths):
    ...
    precision, recall, thresholds = precision_recall_curve(y_true_all,
                                                           y_pred_all)
    if thresholds.size > 0:
        pr_sum = precision[1:] + recall[1:]
        f1 = 2 * (precision[1:] * recall[1:]) / pr_sum
